...
function add_permissons(file, user, permissions, is_allow) {
  ...
  arr.push(["add", [file, user, permissions, is_allow]]);
  emitState();
}

function add_permissons_from_undo(file, user, permissions, is_allow) {
  for (p of permissions) {
    file.acl.push(make_ace(user, p, is_allow));
  }
  emitState();
...
function remove_permissions(file, user, permissions, is_allow) {
  ...
  arr.push(["remove", [file, user, permissions, is_allow]]);
  emitState();
}

function remove_permissions_from_undo(file, user, permissions, is_allow) {
  file.acl = file.acl.filter((ace) => {
    return !(
      ace.who === user &&
      permissions.includes(ace.permission) &&
