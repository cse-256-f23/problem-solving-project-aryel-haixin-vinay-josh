...
function toggle_permission(
  ...
  if (is_on && from_undo) {
    console.log("add from undo");
    add_permissons_from_undo(file, user1, permission1, is_allow);
  } else if (is_on) {
    add_permissons(file_obj, user, [permission], is_allow_ace);
  } else if (from_undo) {
    console.log("remove from undo");
    remove_permissions_from_undo(file, user1, permission1, is_allow);
  } else {
